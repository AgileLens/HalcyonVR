...
void FMetaXRHapticsOpenXRExtension::SetupActionSet()
{
	...
	ensure(ActionSet == XR_NULL_HANDLE);
	...
	if (!NativeSDKInitalized || !OpenXRSessionStarted)
	{
		return;
	}
...
bool FMetaXRHapticsOpenXRExtension::GetRequiredExtensions(TArray<const ANSICHAR*>& OutExtensions)
{
	if (!Module)
	{
		return false;
